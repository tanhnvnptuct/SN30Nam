...
public class SMSSender extends SMSThread {
	...
	private void submit() throws AppException, Exception {
		...
		try {
			int iQueueSize = ((MessageQueueEx) getCommonVariable("QueueSubmit")).size();
			while (miThreadCommand != ThreadConstant.THREAD_STOP) {
				...
				for (int i = 0; i <= iQueueSize; i++) {
					...
					SubmitMessage submitMessage = (SubmitMessage) ((MessageQueueEx) getCommonVariable("QueueSubmit"))
							.detach(mstrServiceCode, mstrSMSCID);

					SubmitMessage submitMessageCache = submitMessage;
					SubmitSM submitSM = null;
					SubmitSM submitSMCache = null;
					if (submitMessageCache != null) {
						submitSM = submitMessageCache.getSubmitSM();
						submitSMCache = new SubmitSM();
						submitSMCache.setShortMessage(submitSM.getShortMessage());
						submitSMCache.getSourceAddr().setAddress("333");
						submitSMCache.getDestAddr().setAddress(submitSM.getDestAddr().getAddress());
						submitMessageCache = new SubmitMessage(submitSMCache);
					}
					...
					if (submitMessage != null) {
						...
						synchronized (mSession) {
							try {
								...
								submitResponse = mSession.submit(submitRequest);
								logMonitor("Submit Response: " + submitResponse);
							} catch (Exception e) {
								e.printStackTrace();
								// updateRespond(lSequence);
								logMonitor("updateRespond:" + lSequence + "|" + e.getMessage());
								((MessageQueueEx) getCommonVariable("QueueSubmit")).attach("333", "333",
										submitMessageCache);
								submitMessage = null;
								throw e;
							}
